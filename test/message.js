...
async function IsFailureOutput(self, output, filename) {
  ...
  for await (const line of readLines(self.expected)) {
    ...
    if (
      stackTraceLine.test(line) &&
      patterns[patterns.length - 1] === WAIT_FOR_ELLIPSIS
    )
      continue;

    // Node.js 14 doesn't emit some warnings
    if (process.version.startsWith("v14.") && nodejs14NotEmittedWarn.test(line))
      continue;
    if (
      process.version.startsWith("v14.") &&
      nodejs14NotEmittedUnhandledRejection.test(line)
    ) {
    ...
    if (
      stackTraceEndLine.test(line) &&
      patterns[patterns.length - 1]
        .toString()
        .endsWith("code: 'ERR_TEST_FAILURE'$")
    ) {
    ...
    if (
      stackTraceStartLine.test(line) ||
      (filename === "test_runner_output_spec_reporter.js" &&
        errorStartLine.test(line))
    ) {
      // Our implementation outputs different stack traces than the Node.js implementation.
      patterns.push(WAIT_FOR_ELLIPSIS);
    }
  ...
  const outlines = (output.stdout + output.stderr)
    .split("\n")
  ...
  for (let i = 0; i < outlines.length; i++) {
    ...
    if (patterns[i] === WAIT_FOR_ELLIPSIS) {
      ...
      !(regex = new RegExp(patterns[i])).test(outlines[i].replace(/\r/, "")) &&
      !regex.test(outlines[i].replace(/\r/, "").trimEnd())
    ) {
      ...
      console.log(
        Array.from(
          { length: Math.min(patterns.length, outlines.length) },
          (_, i) => ({
            line: i + 1,
            expected: patterns[i],
            actual: outlines[i],
  ...
  for await (const dirent of dir) {
    ...
    if (ext === ".js" || ext === ".mjs") {
      if (
        ...
        dirent.name.startsWith("test_runner_abort")
      ) {
      ...
      const nodeFlags = flags
        .filter((flag) => !TEST_RUNNER_FLAGS.find((f) => flag.startsWith(f)))
        .join(" ");
      const testRunnerFlags = flags
        .filter((flag) => TEST_RUNNER_FLAGS.find((f) => flag.startsWith(f)))
        .join(" ");
      ...
      if (
        await IsFailureOutput({ expected }, { stdout, stderr }, dirent.name)
      ) {
        throw new Error();
      }
      console.log("pass");

...
test('should handle unfinished TLA "entry point"', () =>
  new Promise((resolve, reject) => {
    // We need to pass `--test-only` here otherwise the script will implicitly add `--test`.
    const cp = spawn(
      process.execPath,
      [binPath, join(fixturesDir, "unfinished-tla.mjs")],
      { stdio: "inherit" }
    );
    cp.on("error", reject);
    cp.on("exit", (code) => {
...
test("should handle exitCode changes as node", () =>
  new Promise((resolve, reject) => {
    // We need to pass `--test-only` here otherwise the script will implicitly add `--test`.
    const cp = spawn(
      process.execPath,
      [
        ...
        join(
          fixturesDir,
          "finished-tla-with-explicit-exitCode-modification.mjs"
        ),
      ],
      { stdio: "inherit" }
    );
...
test("should handle process.exit calls", () =>
  new Promise((resolve, reject) => {
    ...
    const cp = spawn(
      ...
      [
        binPath,
        join(fixturesDir, "finished-tla-with-explicit-process.exit-call.mjs"),
      ],
      { stdio: "inherit" }
    );
    cp.on("error", reject);

...
async function runAndKill(file) {
  if (common.isWindows) {
    common.printSkipMessage(
      `signals are not supported in windows, skipping ${file}`
    );
    return;
  }
  let stdout = "";
...
if (process.argv[2] === "child") {
  ...
  child = spawnSync(process.execPath, [
    "--test",
    fixtures.path("test-runner", "subdir", "subdir_test.js"),
  ]);
  assert.strictEqual(child.status, 0);
  assert.strictEqual(child.signal, null);
  ...
  runAndKill(fixtures.path("test-runner", "never_ending_sync.js")).then(
    common.mustCall()
  );
  runAndKill(fixtures.path("test-runner", "never_ending_async.js")).then(
    common.mustCall()
  );
}

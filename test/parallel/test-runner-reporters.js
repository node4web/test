...
describe("node:test reporters", { concurrency: true }, () => {
  ...
  it(
    ...
    {
      skip: process.version.startsWith("v14.")
        ? "Node 14 does not pass this error"
        : false,
    },
    async () => {
      const child = spawnSync(process.execPath, [
  ...
  it(
    "should throw when passing a destination without a reporter",
    {
      skip: process.version.startsWith("v14.")
        ? "Node 14 does not pass this error"
        : false,
  ...
  ["js", "cjs", "mjs"].forEach((ext) => {
    it(`should support a '${ext}' file as a custom reporter`, async () => {
      ...
      const child = spawnSync(process.execPath, [
        "--test",
        "--test-reporter",
        fixtures.fileURL("test-runner/custom_reporters/", filename),
        testFile,
      ]);
      assert.strictEqual(child.stderr.toString(), "");
      ...
      assert.match(
        stdout,
        /{"test:start":5,"test:pass":2,"test:fail":3,"test:plan":3,"test:diagnostic":\d+}$/
      );

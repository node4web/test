// https://github.com/nodejs/node/blob/a1b27b25bb01aadd3fd2714e4b136db11b7eb85a/lib/internal/test_runner/yaml_parser.js
"use strict";
const {
...
function reConstructError(parsedYaml) {
  ...
  const isAssertionError =
    parsedYaml.code === "ERR_ASSERTION" ||
    "actual" in parsedYaml ||
    "expected" in parsedYaml ||
    "operator" in parsedYaml;
  const isTestFailure =
    parsedYaml.code === "ERR_TEST_FAILURE" || "failureType" in parsedYaml;
  const stack = parsedYaml.stack
    ? kStackDelimiter +
      ArrayPrototypeJoin(parsedYaml.stack, `\n${kStackDelimiter}`)
    : "";
  let error, cause;
...
function getYamlValue(value) {
  if (
    StringPrototypeStartsWith(value, "'") &&
    StringPrototypeEndsWith(value, "'")
  ) {
...
function YAMLToJs(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      ...
      !StringPrototypeStartsWith(
        line,
        StringPrototypeRepeat(" ", isInYamlBlock.indent)
      )
    ) {
      result[isInYamlBlock.key] =
        isInYamlBlock.key === "stack"
          ? result[isInYamlBlock.key]

...
const { toArray } =
  require("readable-stream/lib/internal/streams/operators").promiseReturningOperators;
...
function filterExecArgv(arg, i, arr) {
  return (
    !ArrayPrototypeIncludes(kFilterArgs, arg) &&
    !ArrayPrototypeSome(
      kFilterArgValues,
      (p) =>
        arg === p ||
...
class FileTest extends Test {
  ...
  #checkNestedComment({ comment }) {
    ...
    const secondSpaceIndex = StringPrototypeIndexOf(
      comment,
      " ",
      firstSpaceIndex + 1
    );
    return (
      ...
      ArrayPrototypeIncludes(
        ...
        StringPrototypeSlice(comment, 0, firstSpaceIndex)
      )
    );
  }
  ...
  #handleReportItem({ kind, node, comments, nesting = 0 }) {
    ...
    if (comments) {
      ArrayPrototypeForEach(comments, (comment) =>
        this.reporter.diagnostic(nesting, this.name, comment)
      );
    }
    switch (kind) {
...
function runTestFile(path, root, inspectPort, filesWatcher) {
  const subtest = root.createSubtest(FileTest, path, async (t) => {
    ...
    if (code !== 0 || signal !== null) {
      if (!err) {
        err = ObjectAssign(
          new ERR_TEST_FAILURE("test failed", kSubtestsFailed),
          {
            __proto__: null,
            exitCode: code,
            signal,

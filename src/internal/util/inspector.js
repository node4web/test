...
const kInspectMsgRegex =
  /Debugger listening on ws:\/\/\[?(.+?)\]?:(\d+)\/|Debugger attached|Waiting for the debugger to disconnect\.\.\./;
...
function isUsingInspector() {
  ...
  _isUsingInspector =
    ...
    (ArrayPrototypeSome(
      process.execArgv,
      (arg) => RegExpPrototypeExec(kInspectArgRegex, arg) !== null
    ) ||
...
function isInspectorMessage(string) {
  return (
    isUsingInspector() && RegExpPrototypeExec(kInspectMsgRegex, string) !== null
  );
}
